...
#if 1
#define VCONF_JSON_DATA VCONFKEY_SETAPPL_SIM2_NAME
#else
#define VCONF_JSON_DATA VCONFKEY_SETAPPL_CALL_RINGTONE_USER_LIST
#endif
...
static void ss_json_ringtone_save(JsonNode *root)
{
	...
	vconf_set_str(VCONFKEY_SETAPPL_SIM2_NAME,(char*)result);
#endif
...
JsonParser* ss_json_ringtone_load_from_data()
{
	...
	error = NULL;
	char* load_data = vconf_get_str(VCONFKEY_SETAPPL_SIM2_NAME);
...
void ss_json_ringtone_print(JsonNode *root)
{
	...
	SETTING_TRACE("VCONFKEY_SETAPPL_SIM2_NAME SET !!!!\n");
	vconf_set_str(VCONFKEY_SETAPPL_SIM2_NAME,(char*)data);
	g_free(data);
	g_object_unref(generator);
}
